...
from dataset import MyDataset, get_loader_pair
from model import get_model
from xz import debug, display_imgs
from ztorch.engine import Engine
from ztorch.plugins import Accuracy, Checkpoint, Loss, Messages, ReduceLROnPlateau, Timestamp, ValidMetrics
...
def plot():
    ...
    for i, (k, df) in enumerate(dfs.items()):
        # plt.plot(
        #     df.index,
        #     df['avg_train_loss'].astype(np.double),
        #     color='red',
        #     label='[BRCA] Avg. training loss',
        # )
        plt.plot(
            df.index,
            df['val_auc'].astype(np.double),
