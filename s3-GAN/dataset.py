...
class MyDataset(tdata.Dataset):
    ...
    def load_and_process_image(self, dicom_id):
        ...
        # when there's only one channel in the image, the image is
        # usually stored with only two dimensions. We need to pad one dimension.
        if len(img.shape) == 2:
...
"""
            ...
            if self.config['phase'] == 'cc':
                #
                # in the meta table
                #
                #    0 = control
                #    1 = screen-detected (TP)
                #    2 = interval (FN)
                #

                # here, we building a model determining whether the sample is interval (1) or not (0)
                label = np.array([1 if row['label'] == 2 else 0], dtype=np.float32)
            elif self.config['phase'] == 'ccc':
                label = (np.arange(3) == row['label']).astype(np.float32)
            else:
                raise ValueError()

            vd = np.array([row['volpara_vd']], dtype=np.float32)

        return id_, data, label, vd
    ...
    def collate_fn(self, batch):
        ...
        label_list = []
        vd_list = []
        for id_, data, label, vd in batch:
            id_list.append(id_)
            data_list.append(data)
            label_list.append(label)
            vd_list.append(vd)

        return id_list, torch.tensor(data_list).transpose(0, 1), torch.tensor(label_list), torch.tensor(vd_list)
