...
class CNN(nn.Module):
    ...
    def __init__(self, n_input_channel):
        super().__init__()
        # create a base model (bm)
        bm = models.densenet121(pretrained=True)

        # modify the first layer of the base model to have one or two input
        # channels instead of three
        tmp = nn.Conv2d(n_input_channel, 64, kernel_size=7, stride=2, padding=3, bias=False)
        sd = bm.features.conv0.state_dict()
        sd['weight'] = sd['weight'][:, list(range(n_input_channel)), :, :]
        tmp.load_state_dict(sd)
        bm.features.conv0 = tmp

        # remove the last dense layer of the base model (in order to have our
        # own)
        bm.classifier = nn.Identity()
...
def get_cnn(config):
    return CNN(n_input_channel=config['n_input_channel'])
